"""
DDPM "Edit Friendly" Inversion-based counterfactual generator.

Uses the stochastic DDPM forward process to create an "edit-friendly" noise space,
then reconstructs with edited graph embedding via ControlNet.

This method provides better structure preservation AND better edit flexibility
compared to DDIM inversion, due to its stochastic nature.

Based on: "An Edit Friendly DDPM Noise Space: Inversion and Manipulations" (CVPR 2024)
https://github.com/inbarhub/DDPM_inversion

Key Parameters:
- skip: Controls fidelity vs edit strength (0-40, default 36)
  - Higher skip = start closer to clean image = more fidelity, less edit
  - Lower skip = start closer to noise = more edit freedom, less fidelity
- cfg_tar: Classifier-free guidance for reconstruction (7-18, default 15)
"""
...
class DDPMInversionGenerator(BaseCounterfactualGenerator):
    """
    Counterfactual generator using DDPM "Edit Friendly" inversion.
    
    Pipeline:
    1. DDPM forward diffusion: progressively add noise to image, storing noise at each step
    2. Edit graph (add/remove pathology node)
    3. Re-encode edited graph
    4. Denoise from timestep T-skip (with edited graph embedding via ControlNet)
    
    The "skip" parameter is key:
    - Start denoising from z_{T-skip} instead of z_T
    - This means starting from a partially noisy image, not pure noise
    - Allows better structure preservation while still enabling meaningful edits
    ...
    Args:
        ...
        guidance_scale: CFG scale for reconstruction (cfg_tar in paper)
    """
    ...
    def __init__(
        ...
        skip: int = 36,  # Key parameter: how many steps to skip
        guidance_scale: float = 15.0,  # cfg_tar in paper (not used in pure graph mode)
        controlnet_scale: float = 1.0,  # Scale for ControlNet conditioning
        ...
        logger.info("Initializing DDPMInversionGenerator...")
        logger.info(f"  skip={skip}, guidance_scale={guidance_scale}, controlnet_scale={controlnet_scale}, use_independent_noise={use_independent_noise}")
        ...
        logger.info("DDPMInversionGenerator initialized successfully")
    ...
    def _load_models(self, controlnet_path: str, pretrained_model: str):
        ...
        # Setup DDPM scheduler for forward process (adding noise)
        ...
        # Setup DDIM scheduler for denoising (faster, deterministic given the starting point)
    ...
    def ddpm_forward_diffusion(
        ...
        This is the "edit friendly" inversion from the paper.
        Key insight: store the normalized noise (zs) at each step, allowing us to:
        1. Reconstruct perfectly by using the same zs during denoising
        2. Edit by starting from an intermediate timestep with different conditioning
        ...
        for t in timesteps:
            ...
            # z = (x_{t-1} - mu_xt) / (eta * sqrt(variance))
    ...
    def edit_friendly_denoise(
        ...
        """
        Denoise starting from timestep T-skip using ControlNet with graph embeddings.
        
        The "skip" parameter is key to the edit-friendly approach:
        - skip=0: Start from pure noise z_T → maximum edit freedom, minimum fidelity
        - skip=T: Start from clean z_0 → no edits, perfect reconstruction
        - skip=36: Start from z_{T-36} → good balance (recommended)
        
        CFG uses original graph for unconditioned branch and edited graph for
        conditioned branch, so guidance_scale amplifies the graph edit effect.
        ...
        Args:
            ...
            guidance_scale: CFG scale - amplifies difference between original and edited
        ...
        # Start from z_{T-skip} (index from the end of trajectory)
        # trajectory: [z_0, z_1, ..., z_T] has length T+1
        # To start from timestep T-skip, we want index -(skip+1)
        ...
        # timesteps[-zs.shape[0]:] in reference, but we want timesteps[skip:]
        ...
        for t in active_timesteps:
            ...
            if guidance_scale > 1.0:
                # Classifier-free guidance with DIFFERENT graph conditions
                # Unconditioned: original (unedited) graph
                ...
                # No CFG - use edited graph directly
                down_block_res_samples, mid_block_res_sample = self.controlnet(
                    noisy_latents=latents,
                    timestep=t,
                    graph_embeddings=edited_graph_embedding,
    ...
    def generate_batch(
        ...
        """
        Generate counterfactual images for a batch using DDPM Edit Friendly inversion.
        ...
        # Step 1: DDPM forward diffusion (edit-friendly inversion)
        ...
        # Both branches use original embedding - no edit, just reconstruction
        ...
        # Step 2b: Edit-friendly denoising with graph edit (counterfactual)
        # CFG amplifies difference: original (uncond) → edited (cond)
        # Use stored zs to maintain structure while changing semantics
        logger.debug(f"Edit-friendly denoising with edited embedding (skip={self.skip})...")
        ...
        return {
            ...
            'metadata': {
                'method': 'ddpm_fixed' if self.use_independent_noise else 'ddpm_edit_friendly',
