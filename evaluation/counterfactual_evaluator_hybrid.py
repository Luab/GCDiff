"""
Counterfactual evaluation for hybrid (graph + text) conditioned image generation.

Evaluates how well the combined ControlNet (graph) + BioViL-T (text) conditioning
performs by:
1. Using HybridDDPMGenerator to produce edited images with both conditioning paths
2. Measuring pathology flip rates and image quality

This method combines:
- ControlNet with graph embeddings (structural control)
- BioViL-T text embeddings (semantic control)
...
from counterfactual import HybridDDPMGenerator
from counterfactual.base import BaseCounterfactualGenerator
from counterfactual.text_inversion import AVAILABLE_TEMPLATE_SETS
...
class HybridCounterfactualEvaluator:
    """
    Evaluates hybrid (graph + text) counterfactual image generation quality.
    
    This class orchestrates the evaluation pipeline:
    1. Iterates through a dataloader
    2. Uses HybridDDPMGenerator to produce counterfactual images
    3. Computes metrics comparing original vs counterfactual
    
    Args:
        generator: HybridDDPMGenerator instance
    ...
    def __init__(
        ...
        logger.info("Initializing HybridCounterfactualEvaluator...")
        ...
        logger.info("HybridCounterfactualEvaluator initialized successfully")
    ...
    def evaluate(
        ...
        logger.info(f"Starting hybrid counterfactual evaluation for {target_pathology}")
        ...
        sample_idx = 0
        for batch_idx, batch in enumerate(tqdm(dataloader, total=total_batches, desc="Generating hybrid counterfactuals")):
            if num_batches is not None and batch_idx >= num_batches:
                break
            
            # Generate counterfactuals using the hybrid generator
            ...
            # Collect reconstructed images if available
    ...
    def _log_summary(self, results: Dict):
        ...
        logger.info("=" * 70)
        logger.info("HYBRID COUNTERFACTUAL EVALUATION SUMMARY")
        logger.info("=" * 70)
...
def collate_with_graphs(batch):
    ...
    # Separate text prompts if present
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate hybrid (graph + text) counterfactual image generation"
    )
    ...
    parser.add_argument("--guidance_scale", type=float, default=15.0,
                        help="Classifier-free guidance scale for text (default: 15.0)")
    parser.add_argument("--controlnet_scale", type=float, default=1.0,
                        help="ControlNet conditioning scale for graph (default: 1.0). "
    ...
    parser.add_argument("--skip", type=int, default=36,
                        help="Skip parameter for DDPM inversion (0-50). "
                             "Higher = more fidelity, less edit strength. Default: 36")
    parser.add_argument("--use_independent_noise", action="store_true",
                        help="Use independent noise sampling at each timestep "
                             "(per 'Edit Friendly DDPM' paper Eq. 6). "
                             "This is the 'hybrid_fixed' mode.")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/counterfactual_hybrid",
...
def main():
    ...
    method_name = "HybridDDPMGenerator (hybrid_fixed)" if args.use_independent_noise else "HybridDDPMGenerator"
    logger.info(f"Creating {method_name}...")
    if args.use_independent_noise:
        logger.info("Using independent noise sampling at each timestep (per 'Edit Friendly DDPM' paper Eq. 6)")
    generator = HybridDDPMGenerator(
        ...
        use_independent_noise=args.use_independent_noise,
    ...
    evaluator = HybridCounterfactualEvaluator(
        generator=generator,
        device=args.device,
